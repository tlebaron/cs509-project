...
public class SearchTrip {
	...
	public Trips search(String teamName) {
		...
		if(retailCustomerPreferences.searchDateType() == DateType.ARRIVAL) {
			trips = searchTripsByArrivalAirport(teamName, date);
		} else {
			trips = searchTripsByDepartureAirport(teamName, date);
		}
		
		return trips;
	}
	
	private Trips searchTripsByDepartureAirport(String teamName, Calendar departureDate) {
		...
		System.out.println("second leg trips: " + collectTwoLeg.size());
		Trips collectThreeLeg = searchThirdLegTrips(teamName, secondLegPossibleTrips);
		System.out.println("third leg trips: " + collectThreeLeg.size());
	...
	private Trips searchTripsByArrivalAirport(String teamName, Calendar arrivalDate) {
		// TODO Auto-generated method stub
		return null;
	}
	...
	public Flights searchDepartingFlightsOn(String teamName, Airport departureAirport, Calendar departureDate) {
		Flights flights = ServerInterface.INSTANCE.getFlights(departureAirport.code(), departureDate, teamName);
		return flights;
	}
}
